...
const emptyMonster = {
  ...
  hdMode: 'auto', hdManual: 1, hpMode: 'average', hpManual: 1, hpBonus: 0, babMode: 'auto', babManual: 0, babExtra: 0,
...
function hdText(m) {
  return `${m.hd}d${m.hdDie}${m.hpBonusTotal ? signed(m.hpBonusTotal) : ''} (${m.hp} PV)`;
}
...
function build35(monster) {
  ...
  const hpFixedBonus = Number(monster.hpBonus || 0);
  const hpBonusTotal = hd * conMod + hpFixedBonus;
  const hpAverage = Math.max(1, Math.round(hd * averageDie(type.hd) + hpBonusTotal));
  ...
  const built = { ...monster, displayName: monster.name?.trim() || 'Criatura sem nome', typePt: type.pt, sizePt: size.pt, hd, hdDie: type.hd, hp, hpBonusTotal, hpFixedBonus, bab, ac, touch, flat, fort, ref, will, initiative, grapple, space: size.space, reach: size.reach, attacks, primaryAttack, fullAttack: fullAttackText(attacks), dpr35, warnings, typeRule: type, roleRule: role, acParts };
...
function App() {
  ...
  return <main className="app-shell"><div className="arcane-glow" /><div className="mx-auto max-w-[1520px] px-3 py-5 sm:px-6 lg:px-8">
    ...
    </aside><section className="right-stage"><div className="diagnostic-card"><div><h2>Diagnóstico da Forja</h2><p>{converted.roleRule.note}</p></div><button onClick={() => navigator.clipboard.writeText(makeTxt(converted))} className="btn ghost"><Copy size={18} /> Copiar bloco</button><div className="metrics-grid"><Metric label="CR alvo" value={converted.cr35Target || '—'} detail={`ND 5E ${converted.cr5e || '—'}`} /><Metric label="HD/PV" value={`${converted.hdLine}`} detail={converted.hpFixedBonus ? `bônus fixo ${signed(converted.hpFixedBonus)}` : `${converted.hp} PV`} /><Metric label="CA" value={converted.ac} detail={`toque ${converted.touch}, surpreso ${converted.flat}`} /><Metric label="Ataque" value={converted.attacks[0] ? signed(converted.attacks[0].bonus) : '—'} detail={`5E ${signed(monster.attack5e)}`} /><Metric label="Dano médio" value={converted.dpr35 ? converted.dpr35.toFixed(1) : '—'} detail={`5E ${monster.dpr5e || '—'}`} /><Metric label="Ações" value={converted.attacks.length} detail={monster.expectedUse} /></div><ul className="warnings">{converted.warnings.map((w) => <li key={w}>{w}</li>)}</ul></div><div className="preview-wrap"><StatBlock monster={converted} innerRef={statRef} /></div></section></div>
  </div></main>;
}
