...
function newAttack(kind = 'melee') {
  return {
    ...
    name: kind === 'ranged' ? 'Arco curto' : 'Cimitarra',
    category: kind,
    ability: kind === 'ranged' ? 'dex' : 'str',
    extra: 0,
    manual: false,
    manualBonus: 0,
    damage: kind === 'ranged' ? '1d4' : '1d6',
    crit: kind === 'ranged' ? 'x3' : '19-20/x2',
    range: kind === 'ranged' ? '18 m' : '',
    count: 1,
    includeFull: true,
    notes: '',
  };
...
const exampleMonster = {
  ...
  attacks: [
    { ...newAttack('melee'), id: 'atk-melee-1', name: 'Cimitarra', ability: 'dex', damage: '1d4-1', crit: '18-20/x2', manual: false, manualBonus: 4 },
    { ...newAttack('ranged'), id: 'atk-ranged-1', name: 'Arco curto', ability: 'dex', damage: '1d4', crit: 'x3', range: '18 m', manual: false, manualBonus: 4 },
  ],
...
const emptyMonster = { ...exampleMonster, name: '', subtype: '', alignment: '', ac5e: 10, hp5e: 1, attack5e: 0, dpr5e: 0, dc5e: 0, cr5e: '1', cr35Target: '1', str: 10, dex: 10, con: 10, int: 10, wis: 10, cha: 10, hdMode: 'auto', hdManual: 1, hpManual: 1, speed: '', armorBonus: 0, shieldBonus: 0, naturalArmor: 0, deflectionBonus: 0, dodgeBonus: 0, acOther: 0, attacks: [], specialAttacks: '', specialQualities: '', traits5e: '', abilityText: '', skills: '', feats: '', environment: '', organization: '', treasure: '', advancement: '', levelAdjustment: '' };
...
const help = {
  ...
  attacks: 'Adicione quantos ataques ou ações quiser. O primeiro vira Ataque; os marcados entram no Ataque Total.',
};
...
function formatAttack(a) {
  ...
  return `${a.name || 'Ataque'} ${signed(a.bonus)} ${mode} (${a.damage || '—'}${crit}${range})`;
}
...
function build35(monster) {
  ...
  const acParts = {
    tamanho: size.ac, Des: dex, armadura: Number(monster.armorBonus || 0), escudo: Number(monster.shieldBonus || 0), natural: Number(monster.naturalArmor || 0), deflexao: Number(monster.deflectionBonus || 0), esquiva: Number(monster.dodgeBonus || 0), outros: Number(monster.acOther || 0),
  };
  ...
  const primaryAttack = attacks[0] || { name: 'Ataque', bonus: 0, category: 'melee', damage: '—', crit: '', range: '' };
  ...
  return { ...monster, displayName: monster.name?.trim() || 'Criatura sem nome', typePt: type.pt, sizePt: size.pt, hd, hdDie: type.hd, hp, bab, ac, touch, flat, fort, ref, will, initiative, grapple, space: size.space, reach: size.reach, attacks, primaryAttack, fullAttack: fullAttackText(attacks), dpr35, warnings, typeRule: type, roleRule: role, acParts };
}
...
function StatBlock({ monster, innerRef }) {
  ...
  return <article ref={innerRef} className="statblock"><header><h2>{monster.displayName}</h2><p>{monster.sizePt} {monster.typePt}{monster.subtype ? ` (${monster.subtype})` : ''}{monster.alignment ? `, ${monster.alignment}` : ''}</p></header><section>
    ...
    <StatLine label="Iniciativa">{signed(monster.initiative)}</StatLine><StatLine label="Deslocamento">{monster.speed || '—'}</StatLine><StatLine label="Classe de Armadura">{monster.ac} ({acText(monster)}), toque {monster.touch}, surpreso {monster.flat}</StatLine><StatLine label="Ataque Base/Agarrar">{signed(monster.bab)}/{signed(monster.grapple)}</StatLine><StatLine label="Ataque">{formatAttack(monster.primaryAttack)}</StatLine><StatLine label="Ataque Total">{monster.fullAttack}</StatLine><StatLine label="Espaço/Alcance">{monster.space}/{monster.reach}</StatLine><StatLine label="Ataques Especiais">{monster.specialAttacks || '—'}</StatLine><StatLine label="Qualidades Especiais">{monster.specialQualities || '—'}</StatLine><StatLine label="Testes de Resistência">Fort {signed(monster.fort)}, Ref {signed(monster.ref)}, Von {signed(monster.will)}</StatLine><StatLine label="Habilidades">For {monster.str}, Des {monster.dex}, Con {monster.typeRule.noCon ? '—' : monster.con}, Int {monster.int}, Sab {monster.wis}, Car {monster.cha}</StatLine><StatLine label="Perícias">{monster.skills || '—'}</StatLine><StatLine label="Talentos">{monster.feats || '—'}</StatLine><StatLine label="Ambiente">{monster.environment || '—'}</StatLine><StatLine label="Organização">{monster.organization || '—'}</StatLine><StatLine label="Nível de Desafio">{monster.cr35Target || monster.cr5e || '—'}</StatLine><StatLine label="Tesouro">{monster.treasure || '—'}</StatLine><StatLine label="Tendência">{monster.alignment || '—'}</StatLine><StatLine label="Progressão">{monster.advancement || '—'}</StatLine><StatLine label="Ajuste de Nível">{monster.levelAdjustment || '—'}</StatLine></section>{monster.abilityText && <div className="ability-text">{monster.abilityText}</div>}</article>;
}
...
function AttackEditor({ attack, index, updateAttack, removeAttack }) {
  return <div className="attack-box"><div className="mb-3 flex items-center justify-between gap-3"><h4>Ataque/Ação {index + 1}</h4><button type="button" className="btn danger !min-h-9 !px-3" onClick={() => removeAttack(attack.id)}><X size={16} /> Remover</button></div><div className="grid gap-4 sm:grid-cols-2"><Field label="Nome" value={attack.name} onChange={(v) => updateAttack(attack.id, 'name', v)} /><SelectField label="Tipo" value={attack.category} onChange={(v) => updateAttack(attack.id, 'category', v)} options={['melee', 'ranged', 'natural', 'touch', 'special']} /></div><div className="grid gap-4 sm:grid-cols-4"><SelectField label="Atributo" value={attack.ability} onChange={(v) => updateAttack(attack.id, 'ability', v)} options={['str', 'dex', 'none']} /><Field label="Bônus extra" type="number" value={attack.extra} onChange={(v) => updateAttack(attack.id, 'extra', v)} /><Field label="Bônus manual" type="number" value={attack.manualBonus} onChange={(v) => updateAttack(attack.id, 'manualBonus', v)} /><Field label="Qtd. no total" type="number" value={attack.count} onChange={(v) => updateAttack(attack.id, 'count', v)} /></div><div className="grid gap-4 sm:grid-cols-3"><Field label="Dano" value={attack.damage} onChange={(v) => updateAttack(attack.id, 'damage', v)} /><Field label="Crítico" value={attack.crit} onChange={(v) => updateAttack(attack.id, 'crit', v)} /><Field label="Alcance/distância" value={attack.range} onChange={(v) => updateAttack(attack.id, 'range', v)} /></div><TextArea label="Notas da ação" rows={2} value={attack.notes} onChange={(v) => updateAttack(attack.id, 'notes', v)} /><div className="grid gap-2 sm:grid-cols-2"><label className="check-row"><input type="checkbox" checked={attack.manual} onChange={(e) => updateAttack(attack.id, 'manual', e.target.checked)} /> Usar bônus manual</label><label className="check-row"><input type="checkbox" checked={attack.includeFull !== false} onChange={(e) => updateAttack(attack.id, 'includeFull', e.target.checked)} /> Entra no Ataque Total</label></div></div>;
}
